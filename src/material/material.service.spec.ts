jest.mock('../config', () => ({
  envs: {
    ...
    serviceBusConnectionString: 'Endpoint=sb://fake.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=fakekey',
  },
}));
...
jest.mock('@azure/storage-blob', () => {
  ...
  const getBlockBlobClient = jest.fn().mockReturnValue({
    ...
    url: 'https://fakeaccount.blob.core.windows.net/materials/file.pdf',
  });
...
describe('MaterialService', () => {
  ...
  describe('guardarMaterial y guardarTags', () => {
    ...
    it('debería agregar el subject como tag adicional cuando se proporciona', async () => {
      ...
      prismaMock.tags.findFirst.mockResolvedValue({ id: 'tag-subj', tag: 'mate' });
      prismaMock.materialTags.create.mockResolvedValue({});

      await service.guardarMaterial(material as any, ['calculo'], 'mate');

      expect(prismaMock.tags.findFirst).toHaveBeenCalledTimes(2); // 'calculo' + 'mate'
      expect(prismaMock.materialTags.create).toHaveBeenCalledTimes(2);
    });
  });
  ...
  describe('enviarNotificacion', () => {
    it('debería enviar un mensaje a la cola de notificaciones con el cuerpo correcto', async () => {
      ...
      (service as any).prisma.usuarios = {
        findUnique: jest.fn().mockResolvedValue({ email: 'test@test.com', nombre: 'Test User' }),
      };

      await service.enviarNotificacion(response, 'user-1', 'archivo.pdf', 'nuevoMaterialSubido');
    ...
    it('debería manejar usuario no encontrado con valores por defecto', async () => {
      ...
      await service.enviarNotificacion(response, 'user-missing', 'file.pdf', 'template');
  ...
  describe('helpers de blob y cola', () => {
    ...
    it('deleteBlobSafe debería loggear cuando se elimina el blob', async () => {
      ...
      const blockBlob = { deleteIfExists: deleteMock };
      const getBlockBlobClient = jest.fn().mockReturnValue(blockBlob);

      (service as any).containerClient = { getBlockBlobClient };
  ...
  describe('validateMaterial', () => {
    ...
    it('debería orquestar subida, envío a IA y manejo de respuesta (caso feliz)', async () => {
      (service as any).checkDuplicateHash = jest.fn().mockResolvedValue(undefined);
      ...
      (service as any).handleResponse = jest.fn().mockResolvedValue({ id: 'test-uuid', title: 'archivo.pdf' });
    ...
    it('debería lanzar error si hay hash duplicado', async () => {
      (service as any).checkDuplicateHash = jest.fn().mockRejectedValue(new Error('Material already exists with same content'));

      await expect(
        service.validateMaterial(pdfBuffer, { title: 'archivo.pdf', userId: 'user-1' } as any, 'archivo.pdf'),
      ).rejects.toThrow('Material already exists with same content');
    });

    it('debería lanzar error si falla uploadAndAnalyze', async () => {
      (service as any).checkDuplicateHash = jest.fn().mockResolvedValue(undefined);
      (service as any).uploadAndAnalyze = jest.fn().mockRejectedValue(new Error('Error almacenando PDF'));
  ...
  describe('handleResponse', () => {
    ...
    it('debería lanzar error y limpiar blob si falla guardarMaterial', async () => {
      ...
      (service as any).guardarMaterial = jest.fn().mockRejectedValue(new Error('DB error'));
      (service as any).deleteBlobSafe = jest.fn().mockResolvedValue(undefined);
  ...
  describe('checkDuplicateHash', () => {
    it('debería pasar si no existe material con el mismo hash', async () => {
      ...
      await expect((service as any).checkDuplicateHash('hash123')).resolves.toBeUndefined();
      expect(prismaMock.materiales.findFirst).toHaveBeenCalledWith({ where: { hash: 'hash123' } });
    });

    it('debería lanzar ConflictException si existe material con el mismo hash', async () => {
      prismaMock.materiales.findFirst.mockResolvedValue({ id: 'existing-mat', hash: 'hash123' });
      await expect((service as any).checkDuplicateHash('hash123')).rejects.toThrow('Material already exists with same content');
    });

    it('debería excluir el material actual cuando se proporciona excludeMaterialId', async () => {
      prismaMock.materiales.findFirst.mockResolvedValue(null);
      await expect((service as any).checkDuplicateHash('hash123', 'current-mat')).resolves.toBeUndefined();
  ...
  // ────────────────────────────
  // NEW TESTS: uploadAndAnalyze
  // ────────────────────────────

  describe('uploadAndAnalyze', () => {
    it('debería subir el blob, enviar mensaje y esperar respuesta', async () => {
      const buffer = Buffer.from('fake pdf');
      const uploadToBlobSpy = jest.fn().mockResolvedValue('https://blob/file.pdf');
      const sendAnalysisMessageSpy = jest.fn().mockResolvedValue(undefined);
      const waitForResponseSpy = jest.fn().mockResolvedValue({ valid: true });

      (service as any).uploadToBlob = uploadToBlobSpy;
      (service as any).sendAnalysisMessage = sendAnalysisMessageSpy;
      (service as any).waitForResponse = waitForResponseSpy;

      const result = await (service as any).uploadAndAnalyze(buffer, 'corr-1', 'file.pdf');

      expect(uploadToBlobSpy).toHaveBeenCalledWith(buffer, 'file.pdf');
      expect(sendAnalysisMessageSpy).toHaveBeenCalledWith('https://blob/file.pdf', 'file.pdf', 'corr-1', 'analysis');
    ...
    it('debería lanzar BadRequestException si falla la subida al blob', async () => {
      const buffer = Buffer.from('fake pdf');
      (service as any).uploadToBlob = jest.fn().mockRejectedValue(new Error('Upload failed'));

      await expect(
        (service as any).uploadAndAnalyze(buffer, 'corr-1', 'file.pdf'),
      ).rejects.toThrow('Error almacenando PDF');
    });

    it('debería lanzar BadRequestException si falla el envío del mensaje a IA', async () => {
      const buffer = Buffer.from('fake pdf');
      (service as any).uploadToBlob = jest.fn().mockResolvedValue('https://blob/file.pdf');
      (service as any).sendAnalysisMessage = jest.fn().mockRejectedValue(new Error('Queue error'));
      (service as any).deleteBlobSafe = jest.fn().mockResolvedValue(undefined);
      ...
      expect((service as any).deleteBlobSafe).toHaveBeenCalledWith('file.pdf', 'corr-1');
    });
  });

  // ────────────────────────────
  // NEW TESTS: validateUserExists
  // ────────────────────────────

  describe('validateUserExists', () => {
    it('debería pasar si el usuario existe', async () => {
      prismaMock.usuarios.findUnique.mockResolvedValue({ id: 'user-1', nombre: 'Test' });
      await expect((service as any).validateUserExists('user-1')).resolves.toBeUndefined();
    });

    it('debería lanzar BadRequestException si el usuario no existe', async () => {
      prismaMock.usuarios.findUnique.mockResolvedValue(null);
      await expect((service as any).validateUserExists('user-missing')).rejects.toThrow('El userId user-missing no existe en la base de datos');
    });
  });

  // ────────────────────────────
  // NEW TESTS: getUserAverageRating
  // ────────────────────────────
  ...
  describe('getUserAverageRating', () => {
    ...
    it('debería lanzar error si el usuario no existe', async () => {
      prismaMock.usuarios.findUnique.mockResolvedValue(null);
      await expect(service.getUserAverageRating('user-missing')).rejects.toThrow();
    });
  });

  // ────────────────────────────
  // NEW TESTS: getUserMaterialsStats
  // ────────────────────────────
  ...
  describe('getUserMaterialsStats', () => {
    ...
    it('debería devolver 0 para calificacionPromedio si no hay calificaciones', async () => {
      ...
      expect(result.totalMateriales).toBe(1);
    });

    it('debería lanzar error si el usuario no existe', async () => {
      prismaMock.usuarios.findUnique.mockResolvedValue(null);
      await expect(service.getUserMaterialsStats('user-missing')).rejects.toThrow();
    });
  });

  // ────────────────────────────
  // NEW TESTS: getTopDownloadedMaterials
  // ────────────────────────────
  ...
  describe('getTopDownloadedMaterials', () => {
    it('debería devolver el top 3 de materiales más descargados', async () => {
      ...
      expect(result.topDownloaded[0].id).toBe('mat-1');
    ...
    it('debería lanzar error si el usuario no existe', async () => {
      prismaMock.usuarios.findUnique.mockResolvedValue(null);
      await expect(service.getTopDownloadedMaterials('user-missing')).rejects.toThrow();
    });
  });

  // ────────────────────────────
  // NEW TESTS: getTopViewedMaterials
  // ────────────────────────────
  ...
  describe('getTopViewedMaterials', () => {
    it('debería devolver el top 3 de materiales más vistos', async () => {
      ...
      expect(result.userId).toBe('user-1');
      expect(result.topViewed).toHaveLength(1);
    ...
    it('debería lanzar error si el usuario no existe', async () => {
      prismaMock.usuarios.findUnique.mockResolvedValue(null);
      await expect(service.getTopViewedMaterials('user-missing')).rejects.toThrow();
    });
  });

  // ────────────────────────────
  // NEW TESTS: getUserTopMaterials
  // ────────────────────────────
  ...
  describe('getUserTopMaterials', () => {
      ...
      prismaMock.materiales.findMany.mockResolvedValue([
        {
          ...
          vistos: 100,
          Calificaciones: [{ calificacion: 5 }],
          MaterialTags: [{ Tags: { tag: 'calculo' } }, { Tags: { tag: 'algebra' } }],
        },
      ]);
    ...
    it('debería lanzar error si el usuario no existe', async () => {
      prismaMock.usuarios.findUnique.mockResolvedValue(null);
      await expect(service.getUserTopMaterials('user-missing')).rejects.toThrow();
    });
  });

  // ────────────────────────────
  // NEW TESTS: getUserTagsPercentage
  // ────────────────────────────
  ...
  describe('getUserTagsPercentage', () => {
    it('debería devolver porcentajes de tags correctos', async () => {
      ...
      expect(result.tags[1].porcentaje).toBeCloseTo(33.33, 1);
    });
    ...
    it('debería lanzar error si el usuario no existe', async () => {
      prismaMock.usuarios.findUnique.mockResolvedValue(null);
      await expect(service.getUserTagsPercentage('user-missing')).rejects.toThrow();
    });
  });

  // ────────────────────────────
  // NEW TESTS: getGlobalTagsPercentage
  // ────────────────────────────
  ...
  describe('getGlobalTagsPercentage', () => {
    it('debería devolver porcentajes globales de tags', async () => {
      ...
      expect(result.tags[0].tag).toBe('calculo');
      expect(result.tags[0].cantidad).toBe(2);
      expect(result.tags[0].porcentaje).toBeCloseTo(66.67, 1);
    });
    ...
    it('debería manejar el caso sin materiales', async () => {
      ...
      expect(result.totalAsociaciones).toBe(0);
      expect(result.tags).toHaveLength(0);
    });
  });

  // ────────────────────────────
  // NEW TESTS: getAllMaterials
  // ────────────────────────────
  ...
  describe('getAllMaterials', () => {
    it('debería devolver todos los materiales con paginación', async () => {
      ...
      // Note: skip=0 is falsy so service converts it to undefined
  ...
  // ────────────────────────────
  // NEW TESTS: searchMaterialsByName
  // ────────────────────────────
  ...
  describe('searchMaterialsByName', () => {
    it('debería buscar materiales por nombre', async () => {
      ...
      expect(prismaMock.materiales.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: {
            nombre: { contains: 'Calculo', mode: 'insensitive' },
          },
        }),
      );
      expect(result).toHaveLength(1);
    ...
    it('debería lanzar BadRequestException con null', async () => {
      await expect(service.searchMaterialsByName(null as any)).rejects.toThrow();
    });
  });

  // ────────────────────────────
  // NEW TESTS: getMaterialsByDate
  // ────────────────────────────
  ...
  describe('getMaterialsByDate', () => {
    ...
    it('debería devolver materiales ordenados por fecha ascendente', async () => {
      prismaMock.materiales.findMany.mockResolvedValue([]);

      await service.getMaterialsByDate('asc');
  ...
  describe('toMaterialDto', () => {
    it('debería mapear correctamente un material con todos los campos', () => {
      ...
      const material = {
        ...
        updatedAt: now,
        MaterialTags: [{ Tags: { tag: 'calculo' } }, { Tags: { tag: 'algebra' } }],
        Calificaciones: [{ calificacion: 4, comentario: 'bueno' }, { calificacion: 5, comentario: '' }],
      ...
      expect(result.nombre).toBe('Test Material');
      expect(result.userName).toBe('Test User');
      expect(result.tags).toEqual(['calculo', 'algebra']);
      expect(result.calificacionPromedio).toBe(4.5);
      expect(result.totalComentarios).toBe(1); // Only 'bueno' is non-empty
    });
  ...
  describe('rateMaterial', () => {
    it('debería registrar una calificación y devolver el promedio', async () => {
      ...
      const result = await service.rateMaterial('mat-1', 'user-1', 5, 'Excelente');

      expect(result.materialId).toBe('mat-1');
      expect(result.rating).toBe(5);
      expect(result.comentario).toBe('Excelente');
      expect(result.calificacionPromedio).toBe(4.5);
      expect(result.totalCalificaciones).toBe(3);
    });

    it('debería lanzar BadRequestException si rating < 1', async () => {
      await expect(service.rateMaterial('mat-1', 'user-1', 0)).rejects.toThrow('La calificación debe estar entre 1 y 5');
    });

    it('debería lanzar BadRequestException si rating > 5', async () => {
      await expect(service.rateMaterial('mat-1', 'user-1', 6)).rejects.toThrow('La calificación debe estar entre 1 y 5');
    });
    ...
    it('debería lanzar NotFoundException si el material no existe', async () => {
      ...
      await expect(service.rateMaterial('mat-missing', 'user-1', 4)).rejects.toThrow('Material no encontrado');
    });

    it('debería lanzar NotFoundException si el usuario no existe', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue({ id: 'mat-1' });
      prismaMock.usuarios.findUnique.mockResolvedValue(null);
      await expect(service.rateMaterial('mat-1', 'user-missing', 4)).rejects.toThrow('Usuario no encontrado');
    });
  ...
  describe('getMaterialRatings', () => {
    it('debería devolver las calificaciones y el promedio', async () => {
      ...
      expect(result.totalDescargas).toBe(10);
      expect(result.totalVistas).toBe(20);
    });
    ...
    it('debería lanzar NotFoundException si el material no existe', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue(null);
      await expect(service.getMaterialRatings('mat-missing')).rejects.toThrow('Material no encontrado');
    });
  });

  // ────────────────────────────
  // NEW TESTS: getMaterialRatingsList
  // ────────────────────────────
  ...
  describe('getMaterialRatingsList', () => {
    it('debería devolver la lista de calificaciones con nombre de usuario', async () => {
      ...
      expect(result[0].comentario).toBe('Excelente');
      expect(result[1].usuarioNombre).toBe('Usuario Anónimo');
      expect(result[1].comentario).toBeNull();
    });

    it('debería lanzar NotFoundException si el material no existe', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue(null);
      await expect(service.getMaterialRatingsList('mat-missing')).rejects.toThrow('Material no encontrado');
    });
  ...
  describe('incrementViews', () => {
    ...
    it('debería lanzar BadRequestException si el material no existe', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue(null);
      await expect(service.incrementViews('mat-missing')).rejects.toThrow('Material con ID mat-missing no encontrado');
    });
  });

  // ────────────────────────────
  // NEW TESTS: incrementDownloads (private)
  // ────────────────────────────
  ...
  describe('incrementDownloads', () => {
    ...
    it('debería lanzar BadRequestException si el material no existe', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue(null);
      await expect((service as any).incrementDownloads('mat-missing')).rejects.toThrow('Material con ID mat-missing no encontrado');
    });
  });

  // ────────────────────────────
  // NEW TESTS: searchMaterials
  // ────────────────────────────
  ...
  describe('searchMaterials', () => {
    it('debería buscar materiales sin filtros', async () => {
      ...
      const result = await service.searchMaterials(undefined, undefined, undefined, undefined, undefined, undefined, 1, 10);

      expect(result.materials).toHaveLength(1);
      expect(result.total).toBe(1);
    });

    it('debería filtrar por palabra clave', async () => {
      prismaMock.materiales.findMany.mockResolvedValue([]);
      prismaMock.materiales.count.mockResolvedValue(0);

      await service.searchMaterials('calculo', undefined, undefined, undefined, undefined, undefined, 1, 10);
      ...
      expect(prismaMock.materiales.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            OR: [
              { nombre: { contains: 'calculo', mode: 'insensitive' } },
              { descripcion: { contains: 'calculo', mode: 'insensitive' } },
            ],
          }),
        }),
      );
    ...
    it('debería filtrar por autor', async () => {
      ...
      await service.searchMaterials(undefined, undefined, 'user-1', undefined, undefined, undefined, 1, 10);
    ...
    it('debería filtrar por tipo de material', async () => {
      prismaMock.materiales.findMany.mockResolvedValue([]);
      prismaMock.materiales.count.mockResolvedValue(0);

      await service.searchMaterials(undefined, undefined, undefined, 'pdf', undefined, undefined, 1, 10);

      expect(prismaMock.materiales.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            extension: { contains: 'pdf', mode: 'insensitive' },
          }),
        }),
      );
    });
    ...
    it('debería filtrar por semestre', async () => {
      ...
      await service.searchMaterials(undefined, undefined, undefined, undefined, 3, undefined, 1, 10);

      expect(prismaMock.materiales.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            descripcion: { contains: '3', mode: 'insensitive' },
          }),
        }),
      );
    });
    ...
    it('debería filtrar por materia (tags) post-fetch', async () => {
      ...
      const result = await service.searchMaterials(undefined, 'calculo', undefined, undefined, undefined, undefined, 1, 10);
    ...
    it('debería filtrar por calificación mínima post-fetch', async () => {
      ...
      const result = await service.searchMaterials(undefined, undefined, undefined, undefined, undefined, 4, 1, 10);

      expect(result.materials).toHaveLength(1);
      expect(result.materials[0].id).toBe('mat-1');
    });

    it('debería excluir materiales sin calificaciones al filtrar por calificación mínima', async () => {
      prismaMock.materiales.findMany.mockResolvedValue([
        {
          id: 'mat-1',
          nombre: 'Material 1',
      ...
      const result = await service.searchMaterials(undefined, undefined, undefined, undefined, undefined, 3, 1, 10);
  ...
  describe('getMaterialStats', () => {
    it('debería devolver las estadísticas de un material', async () => {
      ...
      expect(result.descargas).toBe(50);
    });
  });

  // ────────────────────────────
  // NEW TESTS: getMaterialDetail
  // ────────────────────────────
  ...
  describe('getMaterialDetail', () => {
    it('debería devolver el detalle completo del material', async () => {
      ...
      expect(result.previewURL).toBe('https://blob/detail.pdf');
      expect(prismaMock.materiales.update).toHaveBeenCalledWith({
        where: { id: 'mat-1' },
        data: { vistos: { increment: 1 } },
      });
    ...
    it('debería lanzar NotFoundException si el material no existe', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue(null);
      await expect(service.getMaterialDetail('mat-missing')).rejects.toThrow('Material con id mat-missing no encontrado');
    });
  });

  // ────────────────────────────
  // NEW TESTS: updateMaterialVersion
  // ────────────────────────────
  ...
  describe('updateMaterialVersion', () => {
    it('debería actualizar solo metadata sin archivo', async () => {
      ...
      const result = await service.updateMaterialVersion('mat-1', undefined, 'New Title', 'New desc');
      ...
      expect(prismaMock.materiales.update).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: 'mat-1' },
          data: expect.objectContaining({ nombre: 'New Title' }),
        }),
      );
    });
    ...
    it('debería actualizar con nuevo archivo válido', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue({
        id: 'mat-1',
        nombre: 'Old',
        url: 'https://fakeaccount.blob.core.windows.net/materials/old.pdf',
        hash: 'old-hash',
        extension: 'pdf',
        descripcion: 'Old desc',
        createdAt: new Date(),
      ...
      // Mock internal methods for upload + AI validation
      (service as any).checkDuplicateHash = jest.fn().mockResolvedValue(undefined);
      (service as any).uploadAndAnalyze = jest.fn().mockResolvedValue({
      ...
      prismaMock.materiales.update.mockResolvedValue({
        id: 'mat-1',
        url: 'https://fakeaccount.blob.core.windows.net/materials/new-file.pdf',
        descripcion: 'Updated',
      });

      const pdfBuffer = Buffer.from('%PDF-1.4 new content');
      ...
      expect(result.id).toBe('mat-1');
      expect((service as any).checkDuplicateHash).toHaveBeenCalled();
      expect((service as any).uploadAndAnalyze).toHaveBeenCalled();
    });
    ...
    it('debería lanzar UnprocessableEntityException si IA rechaza el archivo', async () => {
      ...
      (service as any).checkDuplicateHash = jest.fn().mockResolvedValue(undefined);
      (service as any).uploadAndAnalyze = jest.fn().mockResolvedValue({
        blobName: 'bad-file.pdf',
        fileUrl: 'https://blob/bad.pdf',
        response: { valid: false, detalles: 'Invalid content' },
      });
      (service as any).deleteBlobSafe = jest.fn().mockResolvedValue(undefined);

      const pdfBuffer = Buffer.from('%PDF-1.4 bad content');
      await expect(
        service.updateMaterialVersion('mat-1', pdfBuffer, 'Title', 'desc', 'bad.pdf'),
      ).rejects.toThrow('PDF falló la validación automatizada: Invalid content');
    });

    it('debería lanzar UnprocessableEntityException sin motivo si IA rechaza sin detalles', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue({
        id: 'mat-1',
        nombre: 'Old',
        url: 'https://fakeaccount.blob.core.windows.net/materials/old.pdf',
      ...
      const pdfBuffer = Buffer.from('%PDF-1.4 bad content');
      await expect(
        service.updateMaterialVersion('mat-1', pdfBuffer, 'Title'),
      ).rejects.toThrow('PDF falló la validación automatizada');
    });

    it('debería mantener descripción existente si no se proporciona nueva', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue({
        id: 'mat-1',
        nombre: 'Old',
        url: 'https://fakeaccount.blob.core.windows.net/materials/old.pdf',
      ...
      await service.updateMaterialVersion('mat-1', undefined, 'New Title');
  ...
  // ────────────────────────────
  // NEW TESTS: deleteOldBlob
  // ────────────────────────────

  describe('deleteOldBlob', () => {
    it('debería eliminar el blob anterior si la URL existe', async () => {
      const deleteMock = jest.fn().mockResolvedValue({ succeeded: true });
      const getBlockBlobClient = jest.fn().mockReturnValue({ deleteIfExists: deleteMock });
      (service as any).containerClient = { getBlockBlobClient };

      await (service as any).deleteOldBlob(
        'https://fakeaccount.blob.core.windows.net/materials/old-file.pdf',
        'mat-1',
      );

      expect(getBlockBlobClient).toHaveBeenCalled();
      expect(deleteMock).toHaveBeenCalled();
    });
    ...
    it('debería manejar errores silenciosamente', async () => {
      const getBlockBlobClient = jest.fn().mockImplementation(() => {
      ...
      // Should not throw
      await expect(
        (service as any).deleteOldBlob('https://not-a-valid-url', 'mat-1'),
      ).resolves.toBeUndefined();
    });
  });

  // ────────────────────────────
  // NEW TESTS: getMaterialsCount
  // ────────────────────────────
  ...
  describe('getMaterialsCount', () => {
    it('debería devolver la cantidad total de materiales', async () => {
      ...
      expect(prismaMock.materiales.count).toHaveBeenCalledWith();
    });
  ...
  describe('deleteMaterial', () => {
    it('debería eliminar un material y su blob', async () => {
      ...
      expect(deleteOldBlobSpy).toHaveBeenCalledWith(
        'https://fakeaccount.blob.core.windows.net/materials/file.pdf',
        'mat-1',
      );
      expect(prismaMock.materiales.delete).toHaveBeenCalledWith({ where: { id: 'mat-1' } });
    });

    it('debería lanzar NotFoundException si el material no existe', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue(null);
      await expect(service.deleteMaterial('mat-missing')).rejects.toThrow('El material con ID mat-missing no existe');
    });
    ...
    it('debería eliminar material sin URL de blob', async () => {
      ...
      expect(prismaMock.materiales.delete).toHaveBeenCalled();
    });
  });

  // ────────────────────────────
  // NEW TESTS: downloadMaterial
  // ────────────────────────────
  ...
  describe('downloadMaterial', () => {
    it('debería devolver stream, contentType y filename para un material válido', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue({
        id: 'mat-1',
        url: 'https://fakeaccount.blob.core.windows.net/materials/test-file.pdf',
      });
      ...
      (service as any).incrementDownloads = jest.fn().mockResolvedValue(undefined);

      const result = await service.downloadMaterial('mat-1');

      expect(result.contentType).toBe('application/pdf');
      expect(result.filename).toBe('test-file.pdf');
      expect((service as any).incrementDownloads).toHaveBeenCalledWith('mat-1');
    });

    it('debería lanzar BadRequestException si el material no existe', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue(null);
      await expect(service.downloadMaterial('mat-missing')).rejects.toThrow('Material con id mat-missing no existe');
    });

    it('debería intentar fallback con nombre codificado si el blob decodificado no existe', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue({
        id: 'mat-1',
        url: 'https://fakeaccount.blob.core.windows.net/materials/file%20with%20spaces.pdf',
      });

      const existsMock = jest.fn()
        .mockResolvedValueOnce(false)  // decoded name
        .mockResolvedValueOnce(true);  // fallback encoded name
      ...
      const blockBlob1 = { exists: existsMock, download: downloadMock };
      const blockBlob2 = { exists: existsMock, download: downloadMock };

      const getBlockBlobClient = jest.fn()
        .mockReturnValueOnce(blockBlob1)
        .mockReturnValueOnce(blockBlob2);
    ...
    it('debería lanzar NotFoundException si el blob no existe', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue({
        id: 'mat-1',
        url: 'https://fakeaccount.blob.core.windows.net/materials/missing.pdf',
      });

      const existsMock = jest.fn().mockResolvedValue(false);
      const blockBlob = { exists: existsMock };
      const getBlockBlobClient = jest.fn().mockReturnValue(blockBlob);
      (service as any).containerClient = { getBlockBlobClient };

      await expect(service.downloadMaterial('mat-1')).rejects.toThrow('Archivo no encontrado en almacenamiento');
    });
    ...
    it('debería manejar errores internos lanzando BadRequestException', async () => {
      ...
      await expect(service.downloadMaterial('mat-1')).rejects.toThrow('Error obteniendo archivo de almacenamiento');
    });

    it('debería manejar cuando readableStreamBody es null (lanza BadRequestException por import dinámico en test)', async () => {
      prismaMock.materiales.findUnique.mockResolvedValue({
        id: 'mat-1',
        url: 'https://fakeaccount.blob.core.windows.net/materials/test.pdf',
      });

      const existsMock = jest.fn().mockResolvedValue(true);
      const downloadToBufferMock = jest.fn().mockResolvedValue(Buffer.from('pdf content'));
      const downloadMock = jest.fn().mockResolvedValue({
      ...
      // The fallback path uses dynamic import('node:stream') which fails in Jest's VM
      // This results in a BadRequestException since the catch block wraps the error
      await expect(service.downloadMaterial('mat-1')).rejects.toThrow('Error obteniendo archivo de almacenamiento');
    });
  });
});
